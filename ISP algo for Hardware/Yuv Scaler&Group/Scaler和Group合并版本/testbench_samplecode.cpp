...
int main(int argc, char** argv)
{
	...
	cfgname.append(".ini");
	char const* cfgdefault = cfgname.c_str();
	printf("cfg: %s\n", cfgdefault);
	...
	scaler->parse_config(fconfig);
	scaler->updata_config();
	srows = scaler->params.Reg_SrcHeight;
	scols = scaler->params.Reg_SrcWidth;
	drows = scaler->params.Reg_DstHeight;
	dcols = scaler->params.Reg_DstWidth;
	fclose(fconfig);
