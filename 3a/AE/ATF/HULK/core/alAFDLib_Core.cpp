...
void    allib_afd_add_header(void* ptr, float version)
{
    ...
    data->header.struct_version = G_AFD_TUNING_STRUCT_VERSION;
    data->header.lib_version_major = G_AFD_VERSION_MAJOR;
    data->header.lib_version_minor = G_AFD_VERSION_MINOR;
...
void    allib_afd_initial_tuning_data(void *ptr)
{
    ...
    memcpy(data->freq_box, BOX,sizeof(UINT16) * VOTE_NUM );
...
void    _load_default_parameters(AFD_RUNTIME_DATA *runtime_data)
{
    ...
    runtime_data->afd_setting.enable_group = 1;
...
UINT32  _dispatch_avgG_channel_stats(void *g_channel_rows_avg_data, void* g_channel_rows_group, AFD_RUNTIME_DATA *runtime_data)
{
    UINT16 i = 0, j = 0;
    UINT32* avg_G_info = (UINT32*)g_channel_rows_avg_data;
    UINT16* avg_G_group = (UINT16*)g_channel_rows_group;
    ...
    UINT8 group = runtime_data->afd_setting.enable_group;
    if (group)
    {
    ...
    LOG_WHITEBOX("stats downsample line count: %d", tuning_data->jump_sample_num);
    LOG_WHITEBOX("stats downsample avg line num: %d", jump_sample_do_avg_count);
    LOG_STATS("stats downsample line count: %d", tuning_data->jump_sample_num);
    LOG_STATS("stats downsample avg line num: %d", jump_sample_do_avg_count);
    ...
    for (i = 0; i < tuning_data->jump_sample_num; i++)
    {
        ...
        {
            h = i * jump_row_num + avg_jump_index * j;

            if (group)
            {
                /* stats ungroup */
                if (h > sensor->a3engine_height)
                {
                    LOG_DEBUG("group line search: %d, max sensor height is: %d", line, sensor->a3engine_height - 1);
                    return _AL_AFDLIB_PROCESS_GROUP_LINE_OVER_SENSOR_HEIGHT;
                }
                line = avg_G_group[h];
                if (line > sensor->sensor_woi_h)
                {
                    LOG_DEBUG("group line search: %d, max stats line is: %d ", line, sensor->sensor_woi_h - 1);
                    return _AL_AFDLIB_PROCESS_GROUP_LINE_OVER_STATS_HEIGHT;
                }
                //if (avg_G_info[line] > 255)  //8bit
                if (avg_G_info[line] > 1024)  //10bit
                {
                    LOG_DEBUG("stats is over sat,cur stats_G_avg is : %d", avg_G_info[line]);
                    return _AL_AFDLIB_PROCESS_STATS_OVER_SAT;
                }
                sum += avg_G_info[line];
        ...
        stats_data->avg_info[i] = (sum / jump_sample_do_avg_count) >>2;  //8bit
        
        LOG_STATS("STATS line[%d]: %d ",i, stats_data->avg_info[i]);


        //block avg no downsampling while do_avg_count
        //for (j = 0; j < jump_row_num; j++)
        //{
        //      /* no group */
        ////    sum += avg_G_info[i * jump_row_num + j];
        //      /* group */
        //    h = i * jump_row_num + j;
        //    if (h > sensor->a3engine_height)
        //    {
        //        LOG_DEBUG("group line search: %d, max sensor height is: ", line, sensor->a3engine_height - 1);
        //        return _AL_AFDLIB_PROCESS_GROUP_LINE_OVER_SENSOR_HEIGHT;
        //    }
        //    line = avg_G_group[h];
        //    if (line > sensor->sensor_woi_h)
        //    {
        //        LOG_DEBUG("group line search: %d, max stats line is: ",line, sensor->sensor_woi_h-1);
        //        return _AL_AFDLIB_PROCESS_GROUP_LINE_OVER_STATS_HEIGHT;
        //    }
        //    sum += avg_G_info[line];
        //}
    ...
    LOG_WHITEBOX("frame_average: %d ", stats_data->frame_average);
    LOG_STATS("frame_average: %d ", stats_data->frame_average);
...
BOOL    _check_stats_stable(AFD_RUNTIME_DATA *runtime_data)
{
    ...
    LOG_DEBUG("frame_diff is : %d", diff);
...
UINT32  _compute_flicker_range(AFD_RUNTIME_DATA *runtime_data)
{
    ...
    if (runtime_data->afd_setting.enable_group)
    {
        input_height = (0 != sensor->a3engine_height) ? sensor->a3engine_height : input_height;
    }
...
BOOL    _flicker_detection_frame_diff(AFD_RUNTIME_DATA *runtime_data)
{
    ...
    if (0 == max)
    {
        ...
        LOG_DEBUG("_flicker_detection_frame_diff detect_status: %d \n", runtime_data->working_buffer.result.detect_status);
        return TRUE;
    }
...
{
    lib_version->major_version = G_AFD_VERSION_MAJOR;
    lib_version->minor_version = G_AFD_VERSION_MINOR;
}

/* debug level*/
unsigned int allib_atf_set_debug_log_flag(int debug_log_flag, BOOL file_tag, BOOL teminal_tag)
{
    UINT32 errRet = _AL_AFDLIB_SUCCESS;
    if (debug_log_flag < LogLevel_OFF || debug_log_flag >= LogLevel_end)
    {
        errRet = _AL_AFDLIB_SET_PARAM_DEBUG_TYPE_NULL;
        LOG_ERROR("ERROR %x\r", (unsigned int)errRet);
        return errRet;
    }

    set_log_level(debug_log_flag);
...
UINT32  alAFDLib_set_param(struct allib_afd_set_parameter_t *param, void *afd_dat)
{
    ...
    case ALAFD_SET_PARAM_FLICKER_SENSOR_INFO:
        ...
        {
            if (param->para.flicker_sensor_info.freq_step == 0 || param->para.flicker_sensor_info.sample_rate == 0)
            {
                param->para.flicker_sensor_info.freq_step = 7.812;
                param->para.flicker_sensor_info.sample_rate = 4000;
            }
    ...
    case ALAFD_SET_PARAM_ENABLE:
        ...
        runtime_data->afd_setting.enable_group = param->para.afd_setting.enable_group;
    ...
    case ALAFD_SET_PARAM_TUNING_FS:
        ...
        memcpy(runtime_data->tuning_data.freq_box, param->para.fs_tuning_setting.freq_box, sizeof(UINT16)* VOTE_NUM);
...
/* process */
UINT32  alAFDLib_push_stats(void *g_channel_rows_avg_data, void* g_channel_rows_group, void *afd_dat)
{
    ...
    /* stats ungroup */
    if (NULL == g_channel_rows_group)
    {
        return _AL_AFDLIB_PROCESS_STATS_GROUP_NULL;
    }

    runtime_data->output_report.flag_enable = runtime_data->afd_setting.enable;

    err = _dispatch_avgG_channel_stats(g_channel_rows_avg_data, g_channel_rows_group, runtime_data);
...
UINT32  alAFDLib_kernel_process(void *afd_dat, struct allib_afd_output_data_t *output)
{
    ...
    LOG_DEBUG("ATF FS calc on or not : %d", runtime_data->afd_setting.fs_enable);
    ...
    LOG_DEBUG("stable_queue_index : %d ;detect banding : %d ;freq : %d ;stable frequence mode : %d \n", runtime_data->stable_queue_index,output->flag_bending, output->afd_mode,output->afd_mode_stable);
