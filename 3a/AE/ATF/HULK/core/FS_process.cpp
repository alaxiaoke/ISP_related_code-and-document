...
BOOL _flicker_sensor_amp_peak_scan(AFD_RUNTIME_DATA* runtime_data)
{
    ...
    LOG_WHITEBOX("FS peak amp : %d", amp_max);
    LOG_WHITEBOX(" fs peak amp th: %d", th_amp);
    ...
    if (amp_max < th_amp)
    {
        ...
        LOG_WHITEBOX("FS peak amp not enough, detect_status: %d", working->result.detect_status);
        return FALSE;
    }

    LOG_DEBUG("FS peak amp enough");
    LOG_WHITEBOX("FS peak amp enough");
    return TRUE;
...
BOOL _flicker_sensor_amp_pp_ration(AFD_RUNTIME_DATA* runtime_data)
{
    ...
    LOG_WHITEBOX("FS peak amp_max : %d", amp_max);
    LOG_WHITEBOX("FS peak amp2 : %d", amp_max2);
    LOG_WHITEBOX(" fs 2nd peak amp/ amp_max ratio: %f", (amp_max2 << 10) / amp_max / 1024.0);
    LOG_WHITEBOX(" fs peak ratio_peak_2nd th ratio: %d", ratio_peak_2nd);
    LOG_WHITEBOX(" fs 2nd peak amp th: %d", amp_th);
    ...
    if (amp_max2 > amp_th )
    {
        ...
        LOG_WHITEBOX("FS 2nd peak detect_status: %d", working->result.detect_status);
        return FALSE;
    }

    LOG_DEBUG("FS 2nd peak amp not over 1st ratio_th");
    LOG_WHITEBOX("FS 2nd peak amp not over 1st ratio_th");
...
BOOL _flicker_sensor_vote_box(AFD_RUNTIME_DATA* runtime_data)
{
    ...
    LOG_WHITEBOX(" fs vote box delta_min: %d", delta_min);
    LOG_WHITEBOX("freq max is %d", freq);

    // judge freq in box
    for (int i = 0; i < VOTE_NUM; i++)
    {
        //multi[i] = freq / BOX[i];
        multi[i] = freq / runtime_data->tuning_data.freq_box[i];

        INT32 diff_0 = freq - multi[i] * runtime_data->tuning_data.freq_box[i];
        INT32 diff_1 = (multi[i] + 1) * runtime_data->tuning_data.freq_box[i] - freq;
    ...
    if (index == 255)
    {
        ...
        LOG_WHITEBOX("FS box vote failed, stastus: %d, index : %d", working->result.detect_status, index);
        return FALSE;
    }
    LOG_DEBUG("freq in vote box,index is %d", index + 1);
    LOG_WHITEBOX("freq in vote box,index is %d", index + 1);

    working->result.detect_status = FS_DETECT_STATUS_OK;
    //working->result.detect_hz = BOX[index] * multi[index];
    working->result.detect_hz = (enum afd_mode)(index + 1);
...
void _update_fs_stable_queue_report(AFD_RUNTIME_DATA* runtime_data)
{
    ...
    //else
    {
        ...
        LOG_WHITEBOX("fs peak th cnt: %d", runtime_data->tuning_data.fs_th_stable);
