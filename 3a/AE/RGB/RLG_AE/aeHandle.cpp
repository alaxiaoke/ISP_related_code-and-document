...
unsigned int ae_run(gxr_aehd* gxr_ae)
{
    ...
    */
    ...
    //if (gxr_ae->tuning_enable == 1)
    {
        ...
        for (int i = 0; i < maxIaeParamSwitchCase; i++)
        {
            ...
            ae_set_para->set_param.iAE_param.iaeTuningCoreParam[i].max_isp_midtones_gain = paramSim.paramIAEHist[26][i];
        }
        ae_err = ae_func_obj->set_param(ae_set_para, ae_output, *ae_obj);
        if (ae_err != _AL_3ALIB_SUCCESS) {
            return ae_err;
    ...
    LOG_DEBUG("expoSOF2 %f, %f, %f, pipeDelay %u\n", expousureList.front()[0], expousureList.front()[1], expousureList.front()[4], pipeDelay);
    ...
    */
    ...
    gxr_ae->midtones_gain = ae_output->isp_midtones_gain;
    ...
    expoout.push_back(ae_output->midtones_gain);
