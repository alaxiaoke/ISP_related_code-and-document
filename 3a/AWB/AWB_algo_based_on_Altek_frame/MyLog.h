...
#define LOG_PATH "AWBlog"


extern enum LogLevel_em w_emCurLogLevel;
...
static void printf_log(enum LogLevel_em level, const char* file, const char* function, int line, const char* format, ...)
{
	if (w_emCurLogLevel & level)
	{
		...
		sprintf(msg_buf, "[AWB Proc]%02d-%02d %02d:%02d:%02d @[%s] Line:%u:[%s] ", 1 + p->tm_mon, p->tm_mday, p->tm_hour, p->tm_min, p->tm_sec, function, line, log_level_2_str(level));
