...
def fit_filter(train_imgs, data_gt, lambda_regularization, filter_size, u_ticks, v_ticks):
    ...
    print("Minimizing...")
    optionsDict = {'disp': True, "maxiter": 10000}
    res = minimize(loss_fn, np.zeros((filter_size ** 2,)), args=(train_imgs, data_gt, l_array, lambda_regularization, u_ticks, v_ticks), method='L-BFGS-B', options=optionsDict, tol=1e-9)
    return res.x
...
def train(lambda_param):
    ...
    print("Minimizing...")
    res = minimize(loss_fn, np.zeros((25,)), args=(train_imgs, data_gt, l_array, lambda_param, lg.histogram.u_coords, lg.histogram.v_coords), method='CG', tol=1e-6)
    print("Finished!")
    print(res.x)
