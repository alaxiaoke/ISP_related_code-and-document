//					memcpy(gmtabley_, gmtabley, sizeof(gmtabley));
//					lsctable.copyTo(lsctable_);
//					std::thread run[7];
//					for (int k = 0; k < 7; k++)
//					{
//						raw.copyTo(raw_[k]);
//						src_[k] = raw_[k];
//						dst_[k].create(src.rows, src.cols, CV_8UC3);
//						raw2_[k].verbose = raw2.verbose;
//						raw2_[k].rpat = raw2.rpat;
//						raw2_[k].depth = raw2.depth;
//						raw2_[k].bpc = raw2.bpc;
//						memcpy(raw2_[k].ynoise, raw2.ynoise, sizeof(raw2.ynoise));
//						memcpy(raw2_[k].cnoise, raw2.cnoise, sizeof(raw2.cnoise));
//						memcpy(raw2_[k].sharp, raw2.sharp, sizeof(raw2.sharp));
//						memcpy(raw2_[k].bdlarge, raw2.bdlarge, sizeof(raw2.bdlarge));
//						raw2_[k].top = raw2.top;
//						raw2_[k].bottom = raw2.bottom;
//						raw2_[k].left = raw2.left;
//						raw2_[k].right = raw2.right;
//						raw2_[k].wben = raw2.wben;
//						raw2_[k].expen = raw2.expen;
//						memcpy(raw2_[k].wbgain, raw2.wbgain, sizeof(raw2.wbgain));
//						memcpy(raw2_[k].blacklevel, raw2.blacklevel, sizeof(raw2.blacklevel));
//						memcpy(raw2_[k].ccm, raw2.ccm, sizeof(raw2.ccm));
//						raw2_[k].sensorgain = raw2.sensorgain;
//						raw2_[k].ispgain = raw2.ispgain;
//						raw2_[k].adrcgain = raw2.adrcgain;
//						raw2_[k].shutter = raw2.shutter;
//						raw2_[k].luxindex = raw2.luxindex;
//						raw2_[k].lsc_height = raw2.lsc_height;
//						raw2_[k].lsc_width = raw2.lsc_width;
//						raw2_[k].lsctable[0] = (float*)lsctable_.ptr(0);
//						raw2_[k].lsctable[1] = (float*)lsctable_.ptr(1);
//						raw2_[k].lsctable[2] = (float*)lsctable_.ptr(2);
//						raw2_[k].lsctable[3] = (float*)lsctable_.ptr(3);
//						raw2_[k].gmtablex = gmtablex_;
//						raw2_[k].gmtabley = gmtabley_;
//						raw2_[k].height = raw2.height;
//						raw2_[k].width = raw2.width;
//						raw2_[k].input = (ushort*)src_[k].data;
//						raw2_[k].output = dst_[k].data;
//						run[k] = std::thread(&gxr_raw2bgr_run_all, &raw2_[k]);
//					}
...
//						sprintf(key, "_t%d", k);
//						outpre.append(key);
//						cv::imwrite(outpre.append(outext), dst_[k]);
//						fprintf(stdout, "write %s\n", outpre.c_str());
//					}
//				}
